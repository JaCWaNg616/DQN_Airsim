...
class AirSimEnvironment:
    def __init__(self):
        ...
        # 初始化天气和风速
        self._set_weather()
        self._set_random_wind()

    def _set_random_wind(self):
        # 定义八个方向上的风速向量，以及强劲风量
        wind_directions = [
            airsim.Vector3r(2, 0, 0.5),  # 东风
            airsim.Vector3r(-2, 0, 0.5),  # 西风
            airsim.Vector3r(0, 2, 0.5),  # 北风
            airsim.Vector3r(0, -2, 0.5),  # 南风
            airsim.Vector3r(1, 1, 0.5),  # 东北风
            airsim.Vector3r(-1, -1, 0.5),  # 西南风
            airsim.Vector3r(1, -1, 0.5),  # 东南风
            airsim.Vector3r(-1, 1, 0.5),  # 西北风
            airsim.Vector3r(5, 0, 0),  # 东风（强）
            airsim.Vector3r(-5, 0, 0),  # 西风（强）
            airsim.Vector3r(0, 5, 0),  # 北风（强）
            airsim.Vector3r(0, -5, 0),  # 南风（强）
        ]
    ...
    def _set_weather(self):
        ...
        # 设置Dust天气，强度为0.6
        self.client.simSetWeatherParameter(airsim.WeatherParameter.Dust, 0.6)
    ...
    def _compute_reward(self, state):
        ...
        goal_vector = np.array([
            Config.GOAL_POSITION[0] - state[0],
            Config.GOAL_POSITION[1] - state[1],
            Config.GOAL_POSITION[2] - state[2]
        ])

        current_velocity = np.array([velocity.x_val, velocity.y_val, velocity.z_val])
        ...
        # 计算向量之间的夹角
        if goal_magnitude > 0 and velocity_magnitude > 0:
            cos_theta = np.dot(goal_vector, current_velocity) / (goal_magnitude * velocity_magnitude)
            angle_diff = np.arccos(np.clip(cos_theta, -1.0, 1.0))  # 确保 cos_theta 在 [-1, 1] 范围内
        else:
            angle_diff = np.pi  # 如果任一向量为零，假设最大角度差异
    ...
    def reset(self):
        ...
        self.client.moveToZAsync(-1.5, 2).join()
    ...
    def step(self, action):
        ...
        duration = quad_offset[3]/Config.CLOCK_SPEED
